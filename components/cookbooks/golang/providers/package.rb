action :install do
  ...
  bash "Installing package #{new_resource.name}" do
    code "#{node['golang']['install_dir']}/go/bin/go get -v #{new_resource.name} 2> >(grep -v '(download)$' > #{tmp_file_path})"
    action :nothing
...
action :update do
  ...
  bash "Updating package #{new_resource.name}" do
    code "#{node['golang']['install_dir']}/go/bin/go get -v -u #{new_resource.name} 2> >(grep -v '(download)$' > #{tmp_file_path})"
    action :nothing
    user node['golang']['owner']
    group node['golang']['group']
    environment({
