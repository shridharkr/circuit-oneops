    ...
    class Ltm
      ...
      class VirtualServers
        ...
        def refresh_destination_address
	      ...
	      def refresh_connection_limit
          connection_limit = @client['LocalLB.VirtualServer'].get_connection_limit(names)
		      @virtual_servers.each_with_index do |vs, idx|
            vs.connection_limit = connection_limit[idx]
		      end
        end
        ...
        def refresh_all
          ...
          %w(destination_wildmask destination_address type default_pool protocol
             profiles status vlans snat persistence rules connection_limit).each do |item|
            send("refresh_#{item}")
          end
        end
