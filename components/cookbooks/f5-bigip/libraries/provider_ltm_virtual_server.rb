...
class Chef
  class Provider
	...
	@current_resource.connection_limit(vs.connection_limit)
	...
	update_connection_limit unless current_resource.connection_limit == new_resource.connection_limit
      end
      ...
      def create_virtual_server
        converge_by("Create #{new_resource}") do
          ...
          update_current_resource(%w(destination_address destination_port protocol
                                     destination_wildmask type default_pool profiles connection_limit))
          current_resource.default_persistence_profile_cnt = 0
      ...
      def update_connection_limit
        converge_by("Updating #{new_resource} connection_limit to #{new_resource.connection_limit}:#{new_resource.connection_limit}") do
		          Chef::Log.info "Updating #{new_resource} connection_limit"
	load_balancer.client['LocalLB.VirtualServer'].set_connection_limit([new_resource.vs_name], 
									   [{ 'high' => new_resource.connection_limit, 'low' => new_resource.connection_limit }]
									  )
	update_current_resource(%w(connection_limit))
		     new_resource.updated_by_last_action(true)
      end
      end
