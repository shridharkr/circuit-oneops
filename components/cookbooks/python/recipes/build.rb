...
node.set[:python][:build_options] = Mash.new(JSON.parse(node.workorder.rfcCi.ciAttributes.build_options))
configure_options = node[:python][:build_options][:configure]
...
if node.workorder.rfcCi.rfcAction == "add" || (node.workorder.rfcCi.rfcAction == "update" && node.workorder.rfcCi.ciBaseAttributes.has_key?("build_options"))
  version = node[:python][:build_options][:version]
  install_path = "#{node[:python][:build_options][:prefix]}/lib/python#{version.split(/(^\d+\.\d+)/)[1]}"
  
  directory "#{node[:python][:build_options][:srcdir]}" do
    recursive true
    mode 0775
    action :create
  end
