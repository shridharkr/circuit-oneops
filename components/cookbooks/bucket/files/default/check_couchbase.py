...
#mapping a list of resource name we are keeping the state
    ...
    def usage(self):
        print "usage:\n\tcheck_couchbase.py [-l|--list|-h|--help|-b|--bucket|-p|--password]\n\tcheck_couchbase.py metric|all\n\t\tWhere metric is one of the metrics shown with a -l|--list"


    def get_status(self):
        cmd = "/opt/couchbase/bin/cbstats 127.0.0.1:11210 all -b " + self.bucket + " -p " + self.password
        str_stats = subprocess.Popen(cmd,
    ...
    def main(self):
        try:
            opts, args = getopt.getopt(sys.argv[1:], "lhb:p:", ["list", "help","bucket=", "password="])
        ...
        self.before_work()
        for o, a in opts:
            if (o in ("-b")) or (o in ("--bucket")):
               self.bucket = a
            if (o in ("-p")) or (o in ("--password")):
               self.password = a
