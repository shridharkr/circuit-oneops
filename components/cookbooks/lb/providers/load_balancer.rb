...
action :create do
  ruby_block "Create ELB #{new_resource.lb_name}" do
    block do
      ...
      # authorize elb - fog doesnt support port range in authorize_group_and_owner
      g = ec2.security_groups.get(new_resource.security_group)
      g.authorize_group_and_owner('amazon-elb-sg','amazon-elb')
...
action :delete do
  ruby_block "Delete ELB #{new_resource.lb_name}" do
    block do
      ...
      # authorize elb - fog doesnt support port range in authorize_group_and_owner
      g = ec2.security_groups.get(new_resource.security_group)
      g.revoke_group_and_owner('amazon-elb-sg','amazon-elb')
