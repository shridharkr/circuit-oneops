...
action :add do
  unless node.platform != "ubuntu" || ::File.exists?("/etc/apt/sources.list.d/#{new_resource.repo_name}-source.list")
...
action :remove do
  if node.platform == "ubuntu" && ::File.exists?("/etc/apt/sources.list.d/#{new_resource.repo_name}-source.list")
