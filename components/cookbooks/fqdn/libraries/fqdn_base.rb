...
module Fqdn
  ...
  module Base
    ...
    def check_for_hijack_attr_and_record (dns_name,ns)
      is_hijackable = false
      if node.fqdn.hijack_enabled == 'true'
        is_hijackable = true 
      else
        cmd = "dig +short TXT #{dns_name} @#{ns}"  
        Chef::Log.info(cmd)
        vals = `#{cmd}`.split("\n")
        is_hijackable = 'true' if vals.include?("is_hijackable")
      end
    ...
    def get_record_type (dns_name, dns_values)
      ...
      if dns_values.include?("is_hijackable")
        record_type = "txt"
      end
