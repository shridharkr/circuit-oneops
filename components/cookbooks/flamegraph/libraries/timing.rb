...
module BuildEssential
  module Timing
    ...
    def compile_time?
      ...
      !!node["flamegraph"]['build-essential']['compile_time']
    end
    ...
    def check_for_old_attributes!
      unless node['build_essential'].nil?
        Chef::Log.warn <<-EOH
node['build_essential'] has been changed to node['flamegraph']['build-essential'] to match the
...
EOH
        node.default['flamegraph']['build-essential'] = node['flamegraph']['build_essential']
      end

      unless node['flamegraph']['build-essential']['compiletime'].nil?
        Chef::Log.warn <<-EOH
node['flamegraph']['build-essential']['compiletime'] has been deprecated. Please use
node['flamegraph']['build-essential']['compile_time'] instead. I have gracefully converted the
...
EOH
        node.default['flamegraph']['build-essential']['compile_time'] = node['flamegraph']['build-essential']['compiletime']
      end
    end
